...
/** Remove a member. You may remove yourself (leaving the team); a leader may remove anyone.
 *  The member's entries are kept but become unattributed (entries.member_id -> NULL via the
 *  FK's ON DELETE SET NULL — the author_name text stays), so teammates keep the content.
 *  Readiness rows cascade away. If the departing member was the sole leader and others remain,
 *  the oldest remaining member is promoted so the team always has a leader. */
async function removeMember(pool, projectId, targetMemberId, { memberId, secret }) {
  const actor = await requireMember(pool, projectId, memberId, secret);
  const target = targetMemberId || actor.id;
  if (actor.id !== target && actor.role !== "leader") {
    throw httpError(403, "Only the team leader can remove another member");
  }
  const { rows } = await pool.query(`SELECT * FROM members WHERE id = $1 AND project_id = $2`, [target, projectId]);
  const targetRow = rows[0];
  if (!targetRow) throw httpError(404, "Member not found");

  // consolidations.created_by has no ON DELETE rule — detach it first so the delete can proceed.
  await pool.query(`UPDATE consolidations SET created_by = NULL WHERE created_by = $1`, [target]);
  await pool.query(`DELETE FROM members WHERE id = $1 AND project_id = $2`, [target, projectId]);

  if (targetRow.role === "leader") {
    const remaining = await listMembers(pool, projectId);
    if (remaining.length > 0 && !remaining.some((m) => m.role === "leader")) {
      await pool.query(`UPDATE members SET role = 'leader' WHERE id = $1`, [remaining[0].id]);
    }
  }
