...
function addCard(text) {
  ...
  if (typeof syncConnected === "function" && syncConnected()) {
    syncAddEntry(phase, null, text).catch((err) => console.warn("Cloud sync failed:", err.message));
  }
}
...
function initTeamPhasePanel() {
  const cloudConnected = typeof syncConnected === "function" && syncConnected();
  if (!isTeamProject(state) && !cloudConnected) return;
...
function renderTeamPhasePanel() {
  ...
  if (!panel) return;
  if (!isTeamProject(state)) {
    // Cloud-connected but the first pull hasn't landed yet (e.g. a teammate opening this
    // project on a new device) — dtc-team-changed will re-render this once it resolves.
    panel.innerHTML = `<p class="workspace-hint">${t("team.loadingHint")}</p>`;
    return;
  }

  const me = typeof currentIdentity === "function" ? currentIdentity(state) : activeMember(state);
  ...
  panel.querySelectorAll(".team-ready-check").forEach((cb) => {
    cb.addEventListener("change", () => {
      ...
      renderTeamPhasePanel();
      if (typeof syncConnected === "function" && syncConnected()) {
        syncSetReady(phase, cb.checked).catch((err) => console.warn("Cloud sync failed:", err.message));
      }
    });
  });
  ...
  panel.querySelector("#team-approve-btn")?.addEventListener("click", () => {
    if (typeof syncConnected === "function" && syncConnected()) {
      syncApprove(pendingDraft.id).catch((err) => alert(err.message));
      return;
    }
  ...
  panel.querySelector("#team-reject-btn")?.addEventListener("click", () => {
    if (typeof syncConnected === "function" && syncConnected()) {
      syncReject(pendingDraft.id).catch((err) => alert(err.message));
      return;
    }
  ...
  panel.querySelector("#team-reopen-btn")?.addEventListener("click", () => {
    if (typeof syncConnected === "function" && syncConnected()) {
      syncReopen(phase).catch((err) => alert(err.message));
      return;
    }
...
async function consolidatePhase() {
  ...
  try {
    ...
    if (typeof syncConnected === "function" && syncConnected()) {
      await syncConsolidate(phase, text.trim(), cards.map((c) => c.id));
    } else {
