...
async function listMembers(pool, projectId) {
  const { rows } = await pool.query(`SELECT id, name, role FROM members WHERE project_id = $1 ORDER BY created_at`, [
    projectId,
  ]);
  return rows;
}
...
async function createOrJoinTeam(pool, projectId, name) {
  ...
  const { rows } = await pool.query(
    `INSERT INTO members (project_id, name, role, secret) VALUES ($1, $2, $3, $4) RETURNING id, name, role`,
    [projectId, trimmed, role, secret]
  );
...
async function deleteEntry(pool, projectId, entryId, { memberId, secret }) {
  ...
  if (entry.member_id) {
    const member = await requireMember(pool, projectId, memberId, secret);
    if (member.id !== entry.member_id) throw httpError(403, "You can only delete your own entries");
  }
