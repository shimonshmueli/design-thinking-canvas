...
module.exports = withHandler(async (req, res, pool, body) => {
  ...
  if (req.method === "DELETE") {
    // Leave the team (targetMemberId omitted -> remove yourself) or, as leader, remove someone.
    const { memberId, secret, targetMemberId } = body;
    const result = await logic.removeMember(pool, id, targetMemberId, { memberId, secret });
